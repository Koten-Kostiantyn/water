...
!	open(888,file='resm.xls')
	...
	open(893,file='sily1time2tolchek5kolebaniya.txt')

! ispol'zuetsya stolbcy' os' x - pravaya vetka paraboloida obr.xls
! os' u yrad.xls, otsortirovanny'j po vozrastaniyu
!	open(333,file='en.xls')
!     rf0right - tochki na kruge
!     psf0right - svobodnaya poverxnost'
...
!      open(1077,file='time_psf.xls')
      open(299,file='ksi1time2coleb3centrbaka.txt')
...
!      Y=0.
 ...
 !    YSILA=Y
      ...
      RETURN
      END
! tut tolchek rezervuara, s siloj xsila, pryamougol'ny'j tolchek "_|-|_"
      FUNCTION XSILA(T)
	common/period/Per1,akk,YSIL
	COMMON/RS/ RS(6)
	Reak=-0.
	...
	Y=0.
      ...
      IPR=1
!      IPR=0
!     PSZ() - nachal'noe otklonenie po vy'sote
      PSZ(1)=0.25
      PSZ(1)=0.25
      PSZ(2)=0.15
      PSZ(3)=0.15
...
!      PSZ(I)=0.15
   10 CONTINUE
      ...
      NF1=1
      NF2=4
!     Zadaem  funkcii dlya okruzhnogo nomera M=1 i 1-go nomera po radiusu
!     F1=R1*SINX ;F2=R1*COSX
!     dlya okruzhnogo nomera M=1 i 4-go nomera po radiusu:   F9=R4*SINX ;F10=R4*COSX
 	
	CALL VFORMG(H,W,M,NF1,NF2,IPR)
      M=0
      NF1=2
      NF2=6
!     dlya okruzhnogo nomera M=0
!     dlya 2-go radial'nogo nomera F3=R2
!     dlya 6-go radial'nogo nomera F8=R6
      CALL VFORMG(H,W,M,NF1,NF2,IPR)
      M=2
      NF1=3
      NF2=0
!     dlya okruzhnogo nomera M=2 i 3-go nomera po radiusu
!     F4=R3*SIN2X ;F5=R3*COS2X  
      CALL VFORMG(H,W,M,NF1,NF2,IPR)
      M=3
      NF1=5
!     dlya okruzhnogo nomera M=3 i 5-go nomera po radiusu
!     F6=R5*SIN3X ;F7=R5*COS3X  
      CALL VFORMG(H,W,M,NF1,NF2,IPR)
!     NN(10)- nomer funkcii R po radiusu
!     MM(10)- nomer trigonometricheskoj chasti
!     MN(NF1) - okruzhny'e nomera,sootvetstvuyushhie nomeram po radiusu NF1 i NF2
!     F(I)=R(NN(I))*TPS(MM(I))
!     TPS() - opredelyaet trigonometricheskuyu chast' funkcii F
      NN(1)=1
      MM(1)=1
!     Sootvetstvuet M=1;NF1=1;F1=R1*SINX
      NN(2)=1
      MM(2)=2
!     Sootvetstvuet M=1;NF1=1;F2=R1*COSX
      NN(3)=2
      MM(3)=3
!     Sootvetstvuet M=0;NF1=2;F3=R2
      NN(4)=3
      MM(4)=4
!     Sootvetstvuet M=2;NF1=3;F4=R3*SIN2X
      NN(5)=3
      MM(5)=5
!     Sootvetstvuet M=2;NF1=3;F5=R3*COS2X
      NN(6)=6
      MM(6)=3
!     Sootvetstvuet M=0;NF2=6;F6=R6
      NN(7)=5
      MM(7)=6
!     Sootvetstvuet M=3;NF2=5;F7=R5*SIN3X
      NN(8)=5
      MM(8)=7
!     Sootvetstvuet M=3;NF2=5;F8=R5*COS3X
      NN(9)=4
	MM(9)=1
!     Sootvetstvuet M=1;NF2=4;F9=R4*SINX
      NN(10)=4
      MM(10)=2
!     Sootvetstvuet M=1;NF2=4;F9=R4*COSX
      MN(1)=1
!     Sootvetstvuet M=1;NF1=1
      MN(2)=0
!     Sootvetstvuet M=0;NF1=2
      MN(3)=2
!     Sootvetstvuet M=2;NF1=3
      MN(4)=1
!     Sootvetstvuet M=1;NF2=4
      MN(5)=3
!     Sootvetstvuet M=3;NF2=5
      MN(6)=0
!     Sootvetstvuet M=0;NF2=6
      RETURN
      END
...
!   15 FORMAT(3X,F10.4,'=T')
      ...
      !     Zadaem uravnenie obrazuyushhej
...
C     SOME DIFFEFENCIALS FROM GARMONIC POLINOMS FOR GEO-BLOCK
      ...
      COMMON/FORMA/ PSF(25,6),PS0(25),PSZ(6),AF0,AF1
      COMMON/MEN/ MM(10),NN(10),MN(6)
!     FS(I,J,AL,BE,K,NF) - I - nomer proizvodnoj po Z;J - nomer proizvodnoj po R;
!     k - nomer po radiusu; NF - okruzhnoj nomer
      Z=BE*H
      R=AL*FG(Z)
      E=Z-PSZ(K)
!     PSZ(K) - smeshhenie ot iskusstvenno zavy'shennoj vy'soty'
!     E - vy'sota na sreze
!     NF - kol-vo funkcij v razlozhenii ryada(opredelyaet razmer summirovaniya)
...
!     M - okruzhnoj nomer
      L=IJ+M-1
!     Pereschet indeksov dlya garmonicheskix polinomov W
      IF(M.EQ.0) L=L+1
!     Pri nulevom nomere povy'shaem indeks
...
C      IF(I.EQ.3.AND.J.EQ.1) W=WR3Z (M,L,R,E)
    1 S=S+W*PSF(IJ,K)
!     PSF(IJ,K) - massiv, kotory'j opredelyaetsya iz resheniya zadachi na s.znacheniya i sobstv. vektora,
!     vy'chislyaetsya v VFORMG() i opredelyaet samu f(pri i=j=0) i vse proizvodny'e
      FS=S
      RETURN
      END
...
!  655 format('E'pyury' h(96)= y(77)= ', 2F12.8)
!      write (889,302), ALAM
